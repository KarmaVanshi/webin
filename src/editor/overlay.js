...
import { boxModel, measureNeighbours } from './geometry.js';
...
export class Overlay {
  ...
  hide() {
    this.#root.querySelector('.webin-marks').hidden = true;
  }

  show() {
    this.#root.querySelector('.webin-marks').hidden = false;
  }

  /** Hit-tests a pointer event against the resize handles. */
  handleAt(x, y) {
    for (const node of this.#root.querySelectorAll('.webin-handle')) {
  ...
  #render() {
    const marks = this.#root.querySelector('.webin-marks');
  ...
  #renderHover() {
    const box = this.#root.querySelector('.webin-hover');
    const label = this.#root.querySelector('.webin-hover-label');
    ...
    label.hidden = false;
    label.innerHTML =
      `<span class="webin-hover-name">${escapeHtml(truncate(labelFor(element), 34))}</span>` +
      `<span class="webin-hover-size webin-mono">${Math.round(rect.width)} × ${Math.round(rect.height)}</span>`;
    placeLabel(label, rect, this.#view);
  }

  #renderSelection() {
    const layer = this.#root.querySelector('.webin-selection-layer');
    ...
    for (const element of this.#selected.slice(1)) {
      ...
      parts.push(`<div class="webin-sel webin-sel--secondary" style="${styleFor(element.getBoundingClientRect())}"></div>`);
    }

    const rect = primary.getBoundingClientRect();
    parts.push(this.#spacingMarkup(primary, rect));
    parts.push(`<div class="webin-sel" style="${styleFor(rect)}"></div>`);
  ...
  #handleMarkup(rect) {
    return HANDLES.map((h) => {
      ...
      return `<div class="webin-handle" data-handle="${h.id}" data-interactive
  ...
  #dimensionMarkup(element, rect) {
    ...
    return `<div class="webin-dims webin-mono" style="left:${left}px;top:${top}px">${label}</div>`;
  }
  ...
  #spacingMarkup(element, rect) {
    ...
    const margin = `<div class="webin-band webin-band--margin" style="${styleFor({
      left: rect.left - m.left, top: rect.top - m.top,
      width: rect.width + m.left + m.right, height: rect.height + m.top + m.bottom,
    })}"></div>`;

    const padding = `<div class="webin-band webin-band--padding" style="${styleFor({
...
function rule(x, y, width, height, axis, label) {
  ...
  return `<div class="webin-rule" style="${line}"></div>
    <div class="webin-rule-badge webin-mono" style="${badge}">${label}</div>`;
}
...
const TEMPLATE = `
<div class="webin-marks">
  <div class="webin-hover" hidden></div>
  <div class="webin-hover-label" hidden></div>
  <div class="webin-selection-layer"></div>
</div>`;
...
.webin-hover-name { overflow: hidden; text-overflow: ellipsis; min-width: 0; }
.webin-hover-size { opacity: 0.75; flex: none; }
...
.webin-sel--secondary { outline-width: 1px; outline-style: dashed; }

/*
 * A grip is drawn small and hit large.
 *
 * WCAG 2.2 asks for 24 CSS px of pointer target, and a 9px square is not that — but a
 * 24px square drawn over the corner of the element you are trying to see would hide the
 * thing being edited. So the element itself is 24x24 and transparent, and the visible
 * grip is painted inside it. Hit-testing reads the element's box, so what the pointer
 * gets is the full 24px; what the eye gets is the 9px square.
 */
.webin-handle {
  position: absolute;
  width: 24px;
...
.webin-handle:hover::after { background: var(--selection); }
...
.webin-band { position: absolute; pointer-events: none; }
.webin-band--margin { background: rgba(245, 158, 11, 0.16); outline: 1px dashed rgba(245, 158, 11, 0.5); }
.webin-band--padding { background: rgba(34, 197, 94, 0.14); outline: 1px dashed rgba(34, 197, 94, 0.45); }

.webin-rule { position: absolute; background: var(--measure); }
.webin-rule-badge {
  position: absolute;
  padding: 0 3px;
