...
export const KEY = {
  ...
  themes: 'themes',
  site: (host) => `site:${host}`,
  /** Hand edits, kept apart from the theme so either can be cleared without the other. */
  edits: (host) => `edits:${host}`,
};
...
export async function readKey(backend, key, fallback = null) {
  try {
    ...
    console.warn('[webin] storage read failed', key, err);
    return fallback;
  }
}
...
export async function writeKey(backend, key, value) {
  try {
    ...
    console.warn('[webin] storage write failed', key, err);
    return false;
  }
}
...
export async function deleteKey(backend, key) {
  try {
    ...
    console.warn('[webin] storage delete failed', key, err);
    return false;
  }
}
